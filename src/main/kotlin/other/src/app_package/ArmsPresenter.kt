...
fun armsPresenter(isKt: Boolean, provider: ArmsPluginTemplateProviderImpl) = if (isKt) armsPresenterKt(provider) else armsPresenterJava(provider)
...
public class ${provider.pageName.value}Presenter extends BasePresenter<${provider.pageName.value}Contract.Model, ${provider.pageName.value}Contract.View>(model,rootView) {
    @Inject
